...
async function checkoutFormHandler(event) {
  ...
  const streetAddress = document
    .querySelector("#ship-street-checkout")
    .value.trim();
  const postalCode = document
    .querySelector("#ship-postcode-checkout")
    .value.trim();
  const province = document
    .querySelector("#ship-province-checkout")
    .value.trim();
  ...
  const billStreetAddress = document
    .querySelector("#bill-street-checkout")
    .value.trim();
  const billPostalCode = document
    .querySelector("#ship-postcode-checkout")
    .value.trim();
  const billProvince = document
    .querySelector("#ship-province-checkout")
    .value.trim();
  const billCountry = document
    .querySelector("#ship-country-checkout")
    .value.trim();
  ...
  if (ccNumber.length !== 16 || cvc.length !== 3 || !expiry) {
    ...
    if (
      ...
      err_title.innerHTML = "Invalid information";
      err_msg.innerHTML =
        "Please enter your billing address or check the 'Same as shipping address'";
    }
